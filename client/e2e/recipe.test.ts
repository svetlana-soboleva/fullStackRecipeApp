...
const performRecipeFormActions = async (page: any) => {
  await page
...
test.describe.serial('recipe sequence', () => {
  ...
  test('user can update recipe', async ({ page }) => {
    await loginNewUser(page)
    await page.goto('/dashboard')
    const addRecipeBtn = page.getByTestId('createRecipe')
    addRecipeBtn.click()
    ...
    await expect(page.getByTestId('updateBtn')).toBeVisible()

    await page.getByTestId('updateBtn').click()
    await page.getByRole('textbox', { name: 'Step name' }).click()
    await page.getByRole('textbox', { name: 'Step name' }).fill('Update name')

    await page.getByPlaceholder('Ingredients').click()
    await page.getByPlaceholder('Ingredients').fill('New list of ingredients')
    await page.getByRole('button', { name: 'Save' }).click()

    await expect(page.getByTestId('ingredients')).toHaveText(`Update name:New list of ingredients`)
  })
})
